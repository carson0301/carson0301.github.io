...
let isFlipped = false; // 控制鏡頭翻轉
let currentCameraId = null; // 當前使用的攝像頭ID
let availableCameras = []; // 可用攝像頭列表
...
function initializeUI() {
    ...
    document.getElementById('flip-camera').addEventListener('click', toggleCamera);
...
function toggleCamera() {
    ...
    if (webcam && webcam.canvas) {
        ...
        // 應用變換到視頻元素和畫布
        videoElement.style.transform = isFlipped ? 'scaleX(-1)' : 'none';
        canvas.style.transform = isFlipped ? 'scaleX(-1)' : 'none';
        
        // 更新按鈕狀態
        const flipButton = document.getElementById('flip-camera');
        flipButton.textContent = isFlipped ? '關閉鏡頭左右反轉' : '開啟鏡頭左右反轉';
...
async function checkCameraAvailability() {
    try {
        ...
        console.error('檢查攝像頭可用性時出錯:', error);
        return false;
    }
}
...
async function startRecognition() {
    try {
        ...
        webcam = new tmImage.Webcam(500, 400, false); // 寬度, 高度, 翻轉
        
        try {
            await webcam.setup(constraints);  // 請求攝像頭訪問權限
        } catch (setupError) {
        ...
        document.getElementById('switch-camera').disabled = availableCameras.length <= 1;
        ...
        // 應用當前的翻轉狀態
        if (isFlipped) {
            webcam.webcam.style.transform = 'scaleX(-1)';
        ...
        let errorMessage = '無法啟動攝像頭系統。';
        
        // 根據錯誤類型提供具體的錯誤信息
        if (error.message === 'NO_CAMERA') {
            errorMessage = '未檢測到可用的攝像頭設備。請確保您的設備有攝像頭並正常工作。';
...
function updateCurrentValueDisplay(className, confidence) {
    ...
    if (confidence > 0.70) { // 70% 置信度閾值
        ...
        currentValueDisplay.style.color = '#28a745'; // 綠色表示良好的吻合度
    } else if (confidence > 0.5) { // 50-70% 之間
        currentValueDisplay.textContent = `${className} (${(confidence * 100).toFixed(2)}% 吻合度) - 低吻合度`;
        currentValueDisplay.style.color = '#ffc107'; // 黃色表示中等吻合度
    } else {
        currentValueDisplay.textContent = '無法識別紙幣 - 請調整紙幣位置';
        currentValueDisplay.style.color = '#dc3545'; // 紅色表示低吻合度
