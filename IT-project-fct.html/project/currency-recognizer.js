...
async function startRecognition() {
    try {
        ...
        // 設置網絡攝像頭
        webcam = new tmImage.Webcam(500, 400, true); // 寬度, 高度, 翻轉
...
function updateCurrentValueDisplay(className, confidence) {
    ...
    if (confidence > 0.70) { // 70% 置信度閾值
        currentValueDisplay.textContent = `${className} (${(confidence * 100).toFixed(2)}% 信心度)`;
        currentValueDisplay.style.color = '#28a745'; // 綠色表示良好的置信度
    } else if (confidence > 0.5) { // 50-70% 之間
        currentValueDisplay.textContent = `${className} (${(confidence * 100).toFixed(2)}% 信心度) - 低信心度`;
